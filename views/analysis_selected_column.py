			...
			return dbc.Accordion([
				dbc.AccordionItem([
        			...
        			dbc.InputGroup([
        				...
        				dbc.Switch(id='txt_toggle',label="Wyszukaj wiersze, które nie zawierają podanego teksu.",value=False)
        			]),
        		...
        		dbc.AccordionItem([
        			...
        			dbc.InputGroup([
        				...
        				dbc.Switch(id='len_toggle',label="Wyszukaj wiersze, które zawierają tekst o długości innej niż podana.",value=False)
        			]),
        			dbc.Button('Wyszukaj',id='search_len'),
        			html.P(id='result_len')
        		],title="Ile wierszy zawiera tekst o podanej długości?"),
        		dbc.AccordionItem([
        			html.Span('Wybierz wzorzec',id='tooltip',style={"textDecoration":'underline','cursor':'pointer'}),
        			dbc.Tooltip('Za pomocą wzorca możesz wyszukiwać w kolumnie tekstowej np. kody pocztowe, numery telefonów itp.',target='tooltip'),
        			dbc.RadioItems(
        				options=[
        			...
        			dbc.Input(placeholder='Tu wpisz wzorzec zgody z wybraną opcją',type='text',id='pattern'),
        			dbc.FormText('Układy cyfry należy wpisywać używając litery "d" np. dd-ddd' ),
        			dbc.Switch(id='pattern_toggle',label="Wyszukaj wiersze, które zawierają tekst niedopasowany do podanego wzorca.",value=False),
...
def search_length(val1,btn,val2,val3,rows):
	tab_data=pd.DataFrame(rows)
	results_pos=tab_data[tab_data[val2].apply(lambda x: len(x)==val1)].shape[0]
	results_neg=tab_data[~tab_data[val2].apply(lambda x: len(x)==val1)].shape[0]
	btn = dash.callback_context.triggered
...
def search_pattern(val1,val2,val3,val4,btn,rows):
	...
	if btn[0]['prop_id'].split('.')[0] == 'search_pattern':
		if val1==1:
			results_pos=tab_data[tab_data[val4].str.contains(pat='\\b\\w{'+val2+'}\\b',regex=True)].count()[val4]
			results_neg=tab_data[~tab_data[val4].str.contains(pat='\\b\\w{'+val2+'}\\b',regex=True)].count()[val4]
			if val3==False:
				if results_pos==0:
			...
			results_pos=tab_data[tab_data[val4].str.contains(pat='( |\w)*'+val2+'( |\w)*',regex=True)].count()[val4]
			results_neg=tab_data[~tab_data[val4].str.contains(pat='( |\w)*'+val2+'( |\w)*',regex=True)].count()[val4]
			if val3==False:
				if results_pos==0:
			...
			results_pos=tab_data[tab_data[val4].str.contains(pat='^'+val2,regex=True)].count()[val4]
			results_neg=tab_data[~tab_data[val4].str.contains(pat='^'+val2,regex=True)].count()[val4]
			if val3==False:
				if results_pos==0:
